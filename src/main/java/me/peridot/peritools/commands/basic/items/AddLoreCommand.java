...
public class AddLoreCommand extends PeriPlayerCommand {

    private final LangAPI lang;

    public AddLoreCommand(PeriTools plugin) {
        super(plugin, "addlore", "/{LABEL} [dodatkowy opis przedmiotu]", "Dodaje opis do trzymanego w rece przedmiotu", "peritools.cmd.addlore", "dodajopis");
    ...
    public boolean onExecute(Player player, String label, String[] args) {
        ...
        finalLore.addAll(loreList);
        itemMeta.setLore(finalLore);
        item.setItemMeta(itemMeta);
        player.updateInventory();

        this.lang.sendMessage(player, "commands.items.addlore.added", new Replacement("{LORE}", loreString));
